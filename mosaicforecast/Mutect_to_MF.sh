...
mutect_vcf = $1 # mutect VCF filtered with PON and not gzipped
sample_name = $2
output = $3
...
#the above command does some filtering I decided not to do based on the Mutect2 doc recommendations

#simple vcf to bed conversion
cat ${mutect_vcf} | grep -v '^#'| grep -v panel | grep -v PON | grep -v str_contraction | grep -v multiallelic | grep -v t_lod | awk -v sample_name=$sample_name '{OFS="\t"; print $1,$2-1,$2,$4,$5,sample_name}' > ${output}


# rule repeat_filter
# filters out seg dup regions #TODO
